...
const create = async function(reqBody = {}) {
  return await new Transaction(reqBody);
};
...
const createMany = async function(reqBody = [], ordered = false) {
  return await Transaction.insertMany(reqBody, { ordered }, (error, docs) => {
    if (error) {
...
const findAll = async (query = () => {}, timeMS = 1000) => {
  ...
  const foundData = await Transaction.find(query)
    .sort({ created_at: -1 })
    .select("-__v")
    .lean()
    .maxTimeMS(timeMS);
...
const findOne = async (query = () => {}, timeMS = 1000) => {
  console.time("QUERY_TIME");
  const foundData = await Transaction.findOne(query)
    .sort({ created_at: -1 })
    .select("-__v")
    .lean()
    .maxTimeMS(timeMS);
...
const findOneByIdAndUpdate = async (
  ...
  const foundData = await Transaction.findOneAndUpdate(query, update, other);
  return foundData;
};
